...
const metalComponents = ["electric-clay-components"].concat(
	fs.readdirSync("./node_modules").filter(f => f.match(/^clay-.*/))
);
...
module.exports = {
	frontMatterHook: function (data) {
		...
		var iconsDoc = _.get(
			data,
			"index.children.docs.children.patterns.children.icons"
		);
		...
		if (iconsDoc) {
			...
			files = _.reject(files, file => file === "icons.svg");

			files = _.partition(files, file => !_.startsWith(file, "flags-"));
			...
			var flagData = flags.reduce(function (prev, item, index) {
